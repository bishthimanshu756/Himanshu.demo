        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <div x-data="{ show:false}" @click.away="show = false">
                ...
                <div x-show="show" class="absolute bg-gray-500 border-2 border-black-600 font-bold max-h-80 py-4 rounded-md text-white w-52">
                    @if($unenrolledCourses->count())
                        <form method="POST" action="{{ route('users.courses.store', $user) }}">
                            @csrf
                            @foreach($unenrolledCourses as $unenrolledCourse)
                                <div class="block p-2 text-sm">
                                    <input type="checkbox" name="courseIds[]" id="{{ $unenrolledCourse->title }}" value="{{ $unenrolledCourse->id }}">
                                    <label for="{{ $unenrolledCourse->title }}">{{ ($unenrolledCourse->title) }}</label>
                                </div>
                            @endforeach
                ...
                <tbody>
                    @if ($enrolledCourses->count())
                        @foreach($enrolledCourses as $enrolledCourse)
                            <tr class="border-b-2">
                                <td class="text-center py-2">{{ $enrolledCourse->title }}</td>
                                <td class="text-center py-2">
                                    <form method="post" action="{{ route('users.courses.delete', $user) }}">
                                        @csrf
                                        <button name="courseId" value="{{ $enrolledCourse->id }}" type="submit" class="bg-gray-500 font-bold border-2 border-gray-500 hover:bg-gray-700 hover:border-gray-800 hover:text-white px-4 py-1 rounded-md text-white">
                                            {{ __('Unenrolled') }}
                                        </button>
                                    </form>
                                </td>
