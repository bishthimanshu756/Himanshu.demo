                ...
                <tbody>
                    @if($assignedtrainers->count())
                        ...
                        <tr class="border-b-2">
                            ...
                            <td class="text-center py-2">{{ $trainer->email }}</td>
                            <td class="text-center py-2">
                                <form method="post" action="{{ route('users.team.destroy', $user) }}">
                                    @csrf
                                    <button name="trainerId" value="{{ $trainer->id }}" type="submit" class="bg-gray-500 font-bold border-2 border-gray-500 hover:bg-gray-700 hover:border-gray-800 hover:text-white px-4 py-1 rounded-md text-white">{{ __('Unassigned') }}</button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
