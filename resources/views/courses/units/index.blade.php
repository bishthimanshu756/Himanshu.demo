<x-app-layout>
    <div class="mx-24 my-6 py-12">
        ...
        <div class="flex justify-between">
            ...
            <div class="inline">
                <a href="{{ route('courses.units.create', $course) }}" class="bg-blue-500 border font-bold hover:bg-blue-600 px-4 py-2 rounded-md text-sm text-white">{{ __('Add Unit') }}</a>
            </div>
        </div>
        ...
        <div class="px-4 py-2 bg-white">
            <div class="w-full bg-white flex">
                ...
                <div class="w=4/5 relative">
                    <a class="absolute right-6">{{ __('Edit Basic Info') }}</a>
                    <div class="mt-8">
                        <h2 class="font-bold text-2xl text-gray-700">{{ $course->title }}</h2>
                    </div>
                    <span class="font-semibold text-gray-500">{{ $course->description }}</span>
                </div>
            </div>
            <hr>
            <div class="flex justify-between bg-white">
                ...
                <div class="inline">
                    ...
                    <div>{{ $course->updated_at->format('M d, Y') }}</div>
                </div>
                <div class="inline">
                    ...
                    <div>{{ __('Certificate of Completion') }}</div>
                    <div>{{ ($course->certificate == 1) ? __('Yes') : __('No') }}</div>
                </div>
            </div>
        </div>

        <h3 class="font-bold mt-8 text-3xl">Course content</h3>
        
        <!-- Unit Listing -->
        <div class="bg-white mt-8 px-4 py-2">
            @foreach($units as $unit)
                <div class="mt-4 border p-2">
                    <div class="flex justify-between">
                        <span class="text-3xl text-gray-600">{{ $unit->title }}</span>
                        <div class="flex flex-none w-14">
                            <a href="{{ route('courses.units.edit',[$course,$unit]) }}" class="mr-4">
                                <svg class="w-6 h-6 " xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" viewBox="0 0 24 24" fill="currentColor">
                                    <g><rect fill="none" height="24" width="24" x="0"></rect></g><g><g><g><path d="M21,10.12h-6.78l2.74-2.82c-2.73-2.7-7.15-2.8-9.88-0.1c-2.73,2.71-2.73,7.08,0,9.79s7.15,2.71,9.88,0 C18.32,15.65,19,14.08,19,12.1h2c0,1.98-0.88,4.55-2.64,6.29c-3.51,3.48-9.21,3.48-12.72,0c-3.5-3.47-3.53-9.11-0.02-12.58 s9.14-3.47,12.65,0L21,3V10.12z M12.5,8v4.25l3.5,2.08l-0.72,1.21L11,13V8H12.5z"></path></g></g></g>
                                </svg>
                            </a>
                            <a href="{{ route('courses.units.delete', [$course, $unit]) }}" class="mr-2">
